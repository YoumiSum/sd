...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "The width for input images, all the images in the train/validation dataset will be resized to this width. If the width is set, the resolution will be ignored. If both width and height are set, the images will be resized to this width and height. If only width is set, the images will be resized to this width and the height will be set to the same value as weight."
    )
    parser.add_argument(
        "--height",
        type=int,
        default=None,
        help=
        "The height for input images, all the images in the train/validation dataset will be resized to this height. If the height is set, the resolution will be ignored. If both width and height are set, the images will be resized to this width and height. If only height is set, the images will be resized to this height and the width will be set to the same value as height."
    )
...
def main(args):
    ...
    # If passed along, set the training seed now. 随机数种子
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for model in models:
                ...
                elif isinstance(unwrap_model(model),
                                type(unwrap_model(text_encoder_one))):
                    text_encoder_one_lora_layers_to_save = convert_state_dict_to_diffusers(
                        get_peft_model_state_dict(model))
                elif isinstance(unwrap_model(model),
                                type(unwrap_model(text_encoder_two))):
    ...
    def load_model_hook(models, input_dir):
        ...
        if args.train_text_encoder:
            _set_state_dict_into_text_encoder(lora_state_dict,
                                              prefix="text_encoder.",
    ...
    # Preprocessing the datasets.
    # 自适应长宽，而不是仅有resolution同时控制长和宽必须相等
    if args.height is not None and args.width is not None:
        resolution = (args.height, args.width)
    elif args.height is not None:
        resolution = (args.height, args.height)
    elif args.width is not None:
        resolution = (args.width, args.width)
    else:
        resolution = (args.resolution, args.resolution)

    # train_resize的核心是保持图像比例不变，而不是强制resize到resolution
    # 因此需要在train_resize过程中输入短边需要的长度
    short_side = min(resolution)
    train_resize = transforms.Resize(
        short_side, interpolation=transforms.InterpolationMode.BILINEAR)
    train_crop = transforms.CenterCrop(
        resolution) if args.center_crop else transforms.RandomCrop(resolution)
    ...
    def preprocess_train(examples):
        ...
        for image in images:
            ...
            if args.center_crop:
                y1 = max(0, int(round((image.height - resolution[0]) / 2.0)))
                x1 = max(0, int(round((image.width - resolution[1]) / 2.0)))
